...
function Set-TestClipboardText {
    param([string]$Text)

    $deadline = [DateTime]::UtcNow.AddSeconds(10)
    do {
        if ([NativeMenuUi]::SetClipboardText($Text)) {
            return
        }
        if ([DateTime]::UtcNow -ge $deadline) {
            throw "Unable to set clipboard text for menu test"
        }
...
function Select-FirstLink {
    param([IntPtr]$MainWindow)
    Write-Step "select first link"
    [NativeMenuUi]::SendMessage($MainWindow, 0x0100, [IntPtr]0x28, [IntPtr]::Zero) | Out-Null
    Start-Sleep -Milliseconds 120
}
...
function Invoke-MessageCommand {
    ...
    [NativeMenuUi]::PostMessage($MainWindow, 0x0111, [IntPtr]$Command, [IntPtr]::Zero) | Out-Null
    $dialog = Wait-ProcessWindow -Process $Process -ClassName "QuattroThemedMessageDialog" -TitleContains $Title
    if ($Response -eq 1) {
...
"@ | Set-Content -Path (Join-Path $runDir "conf.ini") -Encoding Unicode
    ...
    Select-FirstLink -MainWindow $main
    ...
    Set-TestClipboardText -Text $targetB
    ...
    Close-DialogCommand -Process $process -MainWindow $main -Command 40015 -ClassName "QuattroQuickImportDialog" -Title ""
    ...
    Invoke-CommandImmediate -MainWindow $main -Command 48000
    ...
    if ($process -and !$process.HasExited -and $env:QUATTRO_KEEP_TEST_DIR -ne '1') {
        try {
    ...
    if ($env:QUATTRO_KEEP_TEST_DIR -ne '1') {
