...
function Assert-ControlChecked {
    param([IntPtr]$Parent, [int]$Id, [bool]$Checked, [string]$Name)
    $box = Wait-Control -Parent $Parent -Id $Id
    $actual = [NativePluginToolUi]::SendMessage($box, $BM_GETCHECK, [IntPtr]::Zero, [IntPtr]::Zero).ToInt64() -ne 0
    if ($actual -ne $Checked) {
        throw "Unexpected checked state for $Name control $Id. Expected '$Checked', got '$actual'."
    }
}
...
"@ | Set-Content -Path (Join-Path $runDir "conf.ini") -Encoding UTF8
    ...
    $process = Start-QuattroTestProcess `
        -FilePath (Join-Path $runDir "Quattro.exe") `
        -WorkingDirectory $runDir `
        -Environment @{ QUATTRO_TEST_STOPWATCH_EXPORT = $exportPath }
    ...
    $seenTools = @{}
    for ($command = 47000; $command -lt 47004; ++$command) {
        [NativePluginToolUi]::PostMessage($main, $WM_COMMAND, [IntPtr]$command, [IntPtr]::Zero) | Out-Null
        $toolClass = $(if ($command -eq 47001) { "QuattroClockTool" } else { "QuattroBuiltinTool_*" })
        $tool = Wait-ProcessWindow -Process $process -ClassName $toolClass -TitleContains "" -TimeoutMs 8000
        $title = [NativePluginToolUi]::WindowText($tool)
        $seenTools[$title] = $true

        if ($title -eq "连点器") {
            Assert-Surface -Hwnd $tool -Name "tool-clicker-ready" -Path (Join-Path $LogDir "tool-clicker-ready.png") -RequiredControlIds @(7001,7003,7004,7005,7006,7007,7013,7014,7017)
            ...
            Assert-Surface -Hwnd $tool -Name "tool-clicker-configured" -Path (Join-Path $LogDir "tool-clicker-configured.png") -RequiredControlIds @(7001,7003,7004,7007)
            Invoke-ControlClick -Parent $tool -Id 7007
            Start-Sleep -Milliseconds 700
            Assert-Surface -Hwnd $tool -Name "tool-clicker-complete" -Path (Join-Path $LogDir "tool-clicker-complete.png") -RequiredControlIds @(7001,7003,7004,7007)
            Set-EditText -Parent $tool -Id 7003 -Text "0"
            Assert-Surface -Hwnd $tool -Name "tool-clicker-infinite-configured" -Path (Join-Path $LogDir "tool-clicker-infinite-configured.png") -RequiredControlIds @(7001,7003,7004,7007)
            Invoke-ControlClick -Parent $tool -Id 7007
            Start-Sleep -Milliseconds 350
            Invoke-ControlClick -Parent $tool -Id 7007
            Assert-Surface -Hwnd $tool -Name "tool-clicker-infinite-stopped" -Path (Join-Path $LogDir "tool-clicker-infinite-stopped.png") -RequiredControlIds @(7001,7003,7004,7007)
            [NativePluginToolUi]::PostMessage($tool, $WM_CLOSE, [IntPtr]::Zero, [IntPtr]::Zero) | Out-Null
            Wait-WindowClosed -Hwnd $tool
        } elseif ($title -eq "时钟") {
            Assert-Surface -Hwnd $tool -Name "tool-clock-seconds" -Path (Join-Path $LogDir "tool-clock-seconds.png") -RequiredControlIds @(7801)
            Invoke-ControlClick -Parent $tool -Id 7801
            Assert-ControlChecked -Parent $tool -Id 7801 -Checked $true -Name "clock-milliseconds"
            Assert-Surface -Hwnd $tool -Name "tool-clock-milliseconds" -Path (Join-Path $LogDir "tool-clock-milliseconds.png") -RequiredControlIds @(7801)
            [NativePluginToolUi]::PostMessage($tool, $WM_CLOSE, [IntPtr]::Zero, [IntPtr]::Zero) | Out-Null
            Wait-WindowClosed -Hwnd $tool
            [NativePluginToolUi]::PostMessage($main, $WM_COMMAND, [IntPtr]47001, [IntPtr]::Zero) | Out-Null
            $reopenedClock = Wait-ProcessWindow -Process $process -ClassName "QuattroClockTool" -TitleContains "时钟" -TimeoutMs 5000
            Assert-ControlChecked -Parent $reopenedClock -Id 7801 -Checked $true -Name "clock-reopened-milliseconds"
            [NativePluginToolUi]::PostMessage($reopenedClock, $WM_CLOSE, [IntPtr]::Zero, [IntPtr]::Zero) | Out-Null
            Wait-WindowClosed -Hwnd $reopenedClock
        } elseif ($title -eq "计时器") {
    ...
    foreach ($required in @("连点器", "时钟", "计时器", "秒表")) {
