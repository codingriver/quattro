...
public static class NativeDialogUi {
    ...
    public static bool IsTabButtonSelected(IntPtr hwnd) {
        return GetProp(hwnd, "QuattroThemedControlSelected") != IntPtr.Zero;
    }
    ...
    private static bool Contains(DialogRect outer, DialogRect inner) {
    ...
    private static bool IsLabelInteractiveOverlap(ChildInfo left, ChildInfo right, int minimumArea) {
        ...
        if ((leftLabel && right.ClassName == "Button" && Contains(right.Rect, left.Rect)) ||
            (rightLabel && left.ClassName == "Button" && Contains(left.Rect, right.Rect))) {
            return false;
        }
...
function Assert-DialogSurface {
    ...
    foreach ($id in $RequiredControlIds) {
        ...
        while ([DateTime]::UtcNow -lt $deadline) {
            $child = [NativeDialogUi]::FindDescendantById($Hwnd, $id)
...
function Assert-RadioSelectionGroup {
    ...
    do {
        ...
        foreach ($id in $Ids) {
            ...
            if ([NativeDialogUi]::SendMessage($child, 0x00F0, [IntPtr]::Zero, [IntPtr]::Zero) -eq [IntPtr]1 -or
                [NativeDialogUi]::IsTabButtonSelected($child)) {
                $checked++
            }
        }
        if ($checked -eq $ExpectedChecked) {
            return
        }
        Start-Sleep -Milliseconds 100
    } while ([DateTime]::UtcNow -lt $deadline)
    ...
    if ($checked -ne $ExpectedChecked) {
        $details = foreach ($id in $Ids) {
            $child = [NativeDialogUi]::GetDlgItem($Parent, $id)
            if ($child -eq [IntPtr]::Zero) {
                "id=$id hwnd=0"
            } else {
                $check = [NativeDialogUi]::SendMessage($child, 0x00F0, [IntPtr]::Zero, [IntPtr]::Zero)
                $selected = [NativeDialogUi]::IsTabButtonSelected($child)
                "id=$id hwnd=$child class=$([NativeDialogUi]::ClassName($child)) text=$([NativeDialogUi]::WindowText($child)) check=$check selected=$selected visible=$([NativeDialogUi]::IsWindowVisible($child)) enabled=$([NativeDialogUi]::IsWindowEnabled($child))"
            }
        }
        $details | Set-Content -Path (Join-Path $LogDir ("p2-" + $Name + "-checks.txt")) -Encoding UTF8
...
bHideOnStart=0
bHideNotify=1
"@ | Set-Content -LiteralPath (Join-Path $runDir "conf.ini") -Encoding Unicode
    ...
    Assert-DialogSurface -Hwnd $todoDialog -Name "todo-dialog" -Screenshot (Join-Path $LogDir "p2-dialog-todo.png") -RequiredControlIds @(101,102,150,160,161,162,163,164,165,240)
    Assert-RadioSelectionGroup -Parent $todoDialog -Name "todo-repeat" -Ids @(160,161,162,163,164,165)
    ...
    [NativeDialogUi]::SendMessage($todoDialog, 0x0111, [IntPtr]164, [IntPtr]::Zero) | Out-Null
    Assert-DialogSurface -Hwnd $todoDialog -Name "todo-dialog-fixed" -Screenshot (Join-Path $LogDir "p2-dialog-todo-fixed.png") -RequiredControlIds @(101,102,150,160,164,200,202,240)
    Assert-RadioSelectionGroup -Parent $todoDialog -Name "todo-repeat-monthly" -Ids @(160,161,162,163,164,165)
    ...
    [NativeDialogUi]::SendMessage($todoDialog, 0x0111, [IntPtr]240, [IntPtr]::Zero) | Out-Null
    Assert-DialogSurface -Hwnd $todoDialog -Name "todo-dialog-advanced" -Screenshot (Join-Path $LogDir "p2-dialog-todo-advanced.png") -RequiredControlIds @(101,102,150,160,164,200,202,240,250,252,253)
    [NativeDialogUi]::SendMessage($todoDialog, 0x0111, [IntPtr]160, [IntPtr]::Zero) | Out-Null
    ...
    $quickImportDialog = Wait-ProcessWindow -Process $process -ClassName "QuattroQuickImportDialog"
    Assert-DialogSurface -Hwnd $quickImportDialog -Name "quick-import-dialog" -Screenshot (Join-Path $LogDir "p2-dialog-quick-import.png") -RequiredControlIds @(1001,1002,1003,1004,1005,1,2)
    [NativeDialogUi]::PostMessage($quickImportDialog, 0x0010, [IntPtr]::Zero, [IntPtr]::Zero) | Out-Null
    Wait-WindowClosed -Hwnd $quickImportDialog
    ...
    "screenshots=$mainSmall;$mainLarge;p2-dialog-text.png;p2-dialog-link.png;p2-dialog-todo.png;p2-dialog-todo-fixed.png;p2-dialog-todo-advanced.png;p2-dialog-quick-import.png;p2-dialog-settings.png"
} finally {
