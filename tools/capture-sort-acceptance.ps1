...
function Start-AppWithMode($Mode) {
  ...
  [void][Win32Shot]::ShowWindow($hwnd, 9)
  [void][Win32Shot]::SetWindowPos($hwnd, [IntPtr]::Zero, 80, 80, 420, 520, 0x0040)
  [void][Win32Shot]::SetForegroundWindow($hwnd)
  Start-Sleep -Milliseconds 800
  $rect = New-Object Win32Shot+RECT
  [void][Win32Shot]::GetWindowRect($hwnd, [ref]$rect)
  return @{ Process=$proc; Hwnd=$hwnd; Rect=$rect }
}
...
function Stop-App($Session) {
  ...
  if ($Session -and -not $Session.Process.HasExited) { $Session.Process.Kill() }
}
...
$x = $s.Rect.Left + 350
$y = $s.Rect.Top + 285
[void][Win32Shot]::SetForegroundWindow($s.Hwnd)
