...
function Start-CaseProcess {
    ...
    return Start-QuattroTestProcess -FilePath (Join-Path $RunDir "Quattro.exe") -WorkingDirectory $RunDir
}
...
try {
    ...
    try {
        ...
        Assert-KeyValue -Path $behaviorReport -Name "hide_main_after_tool_open" -Expected "1"
