...
bool SameRows(const std::vector<ThemedFileTransferRow>& a, const std::vector<ThemedFileTransferRow>& b) {
    if (a.size() != b.size()) return false;
    for (std::size_t i = 0; i < a.size(); ++i) {
        if (a[i].id != b[i].id || a[i].fileName != b[i].fileName ||
            a[i].absolutePath != b[i].absolutePath || a[i].size != b[i].size ||
...
void ThemedFileTransferQueueDialog::LayoutControls() {
    ...
    y += ui.labelHeight() + layout.rowGap;
...
void ThemedFileTransferQueueDialog::Refresh() {
    ...
    if (!hasSnapshot_ || !SameRows(snapshot.rows, lastSnapshot_.rows)) {
        std::vector<ThemedTableRow> rows;
        rows.reserve(snapshot.rows.size());
        for (const auto& row : snapshot.rows) {
            rows.push_back(ThemedTableRow{static_cast<std::intptr_t>(row.id), {
                ThemedTableCell{row.fileName}, ThemedTableCell{row.absolutePath},
                ThemedTableCell{FormatByteSizeForDisplay(row.size)}, ThemedTableCell{StatusText(row.status)}}, false, true});
        }
