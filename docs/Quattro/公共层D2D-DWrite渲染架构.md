...
Tooltip、Toast 和主窗口待办提醒统一使用公共 `ThemedWindowUi`。D2D 可用时采用 `WS_EX_LAYERED + UpdateLayeredWindow` 的预乘 alpha surface；透明 surface 使用 grayscale text antialiasing，避免 ClearType 彩边。D2D 不可用时才退回非 layered 的隔离 GDI 绘制。
