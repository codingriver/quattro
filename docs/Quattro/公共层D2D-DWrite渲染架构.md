...
日历单元格使用公共 `calendarDay` 组件。`today` 表示当前日期，采用表面背景、强调色文字和细强调色描边；`selected` 表示用户实际选择的日期，采用实心强调色背景与反白文字。两种业务状态不得复用同一视觉状态；当选中日期等于当前日期时以 `selected` 为主。

时钟、计时器和秒表的主时间值统一使用公共 `timeDisplay` 组件。该组件从当前主题对话框字体派生字体，默认按 `fontScale=5` 放大，并由公共层测量文本、管理 DPI 字体重建、面板内边距、边框和圆角；业务窗口只能提供时间文本和公共布局计算后的可用区域。日期等辅助信息继续使用普通 `Label`，不得随主时间字号放大。

SplitButton 的下拉段由公共按钮运行时保存强类型 `TablerIconId::ChevronDown`，在 owner-draw 阶段按控件 DPI 和当前 normal/hover/pressed/disabled 主题状态直接绘制 glyph。禁止先生成固定颜色的大尺寸 HICON 再缩小显示；字体绘制不可用时，公共层使用同色同尺寸的折线 chevron 后备绘制。
