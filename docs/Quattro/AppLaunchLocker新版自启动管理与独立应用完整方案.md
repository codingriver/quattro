...
│ 应用                   入口来源                       入口  状态      详情  操作   │
├──────────────────────────────────────────────────────────────────────────────────┤
│ [图标] OneDrive        注册表 / 计划任务              3个   已启用    详情  禁用   │
│ [图标] Example App     注册表 / 服务 / 任务           4个   部分禁用  详情  管理   │
│ [图标] New Tool        启动目录                       1个   新增      详情  禁用   │
│ [图标] System Helper   服务 / 系统高级项              2个   仅查看    详情   —     │
│                                                                                    │
│ [图标] Old Client      注册表 / 启动目录              2个   已禁用    详情  恢复   │
├──────────────────────────────────────────────────────────────────────────────────┤
...
|---|---|
| 应用 | 应用图标 + 应用显示名 |
| 入口来源 | 注册表、启动目录、服务、计划任务等来源摘要；不在主列表显示完整应用路径 |
| 入口 | `N 个`，可点击打开详情 |
...
| 风险摘要 | 可理解的来源和风险摘要；完整路径或命令只在详情中显示 |
| 状态 | 仅查看、异常、状态未知 |
| 详情 | 打开来源详情 |
| 操作 | 默认不可操作 |
