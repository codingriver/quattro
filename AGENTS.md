...
- 仓库脚本按用途分层：根目录 `build.ps1` 是本地打包和正式发布的唯一 PowerShell 入口；`tools/` 只放构建期生成器、资源/字体处理、源码规则检查和对应配置（例如 Tabler 图标清单）；所有 PowerShell 测试脚本、验收脚本、截图脚本和测试 harness 必须放在 `tests/scripts/`，不得再新增到 `tools/`。
...
- 本地打包、人工验收打包和 CI 发布打包都必须通过仓库根目录 `build.ps1` 进入；禁止在 `tools/` 下恢复旧打包入口或新增其它打包入口。测试脚本需要触发打包时，也应从 `tests/scripts/` 上溯到仓库根目录后调用 `build.ps1`。
...
- 检查问题、排查 bug 或做自动化测试诊断时，应优先通过日志、运行期打点、语义状态输出和测试进程内记录来确认行为；代码分析无法确认时，也应先补充或读取日志进行检查，其次才使用后台 HWND 截图等视觉证据。截图用于确认可视状态、布局和像素结果，不能替代更直接的语义日志结论。
- 最终验收应优先选择能直接证明需求达成且不干扰桌面的方式；在满足后台截图规则且目标可通过截图可靠证明时，应优先保留截图作为验收证据，再辅以必要日志或测试输出。纯功能向需求仍必须通过相关自动化测试验收，截图只能作为补充证据。
- 除非需求或验收说明明确标记为“交互式视觉验收”，所有自动化测试、自动化截图、测试辅助程序和被测进程一律按非交互后台任务处理；“需要观察界面”“截图工具只能截前台”“现有测试脚本会激活窗口”等均不构成例外。无法在不干扰当前桌面的前提下可靠验收时，必须停止该项自动化验收并报告限制，禁止自动降级为前台、置顶或真实桌面输入方案。
- 自动化测试的后台限制只约束测试执行方式和自动化测试专用入口，不得据此修改正式产品语义或正式打包产物。涉及“全局快捷键唤起主窗口后获得焦点并显示在桌面最前面”的验收，自动化测试应通过进程内公共命令、语义状态接口或隔离测试入口验证唤起意图和状态转换。
- 真实全局快捷键、操作系统前台仲裁以及主窗口最终 Z 序不能在普通后台自动化测试中验收；此类端到端验收必须明确标记为“交互式视觉验收”，默认不得在用户当前桌面运行，只能在独立、无人使用的 Windows 交互会话中执行。禁止以进程内模拟消息、测试专用状态或后台截图结果替代对真实全局快捷键和最终 Z 序的交互式验收结论。
- 自动化测试和自动化截图验收必须默认静默后台运行：测试窗口必须从创建、显示、状态切换、截图到销毁的整个生命周期都保持不激活，并始终位于所有非测试应用窗口之后；不得调用 `SetForegroundWindow`、`SetActiveWindow`、`SetFocus`、`BringWindowToTop`、`SwitchToThisWindow`，不得使用 `HWND_TOPMOST`、`HWND_TOP`、普通 `SW_SHOW` 或其它会抢占前台、焦点、输入法、任务栏注意状态或提升 Z 序的方式。测试需要可见 HWND 时必须使用 `WS_EX_NOACTIVATE`、`SW_SHOWNOACTIVATE` 和 `HWND_BOTTOM` 等后台语义，并在创建后及关键状态变化后重新确认其仍处于后台；测试开始前应记录原前台窗口及焦点归属，测试结束后必须验证二者未因测试发生变化，禁止通过再次抢焦点的方式“恢复”。
- 自动化测试和自动化截图验收不得读取、移动、点击或依赖真实鼠标位置、光标形态、按键状态或桌面输入历史，包括 `GetCursorPos`、`GetCursorInfo`、`GetMessagePos`、`GetAsyncKeyState`、`GetKeyState`、`GetKeyboardState`、`SetCursorPos`、`SendInput`、`mouse_event` 等桌面输入接口。允许的“模拟”仅限验收进程内向明确的测试 HWND 发送固定客户区坐标消息、调用公共语义状态接口或使用专用测试 registry；不得把模拟事件发送到正式版窗口、当前前台窗口或任意非测试进程。Hover、pressed、选择、拖拽等状态必须通过上述进程内方式驱动；Tooltip 视觉验收必须在验收进程内直接调用公共 `ShowTooltip` 并传入固定屏幕坐标，Tooltip hover 绑定行为由 `WM_MOUSEMOVE` 配合测试 registry 的单元测试验证。
...
- 测试启动必须使用独立测试运行根目录、独立配置目录和测试单例标识；测试 harness 必须设置 `QUATTRO_USER_CONFIG_DIR`、`QUATTRO_TEST_MODE`、`QUATTRO_TEST_NO_FOCUS`、`QUATTRO_ACCEPTANCE_MODE` 和 `QUATTRO_TEST_RUN_ID` 等隔离环境变量。测试清理只能按本次测试记录的 PID 和带标记的测试目录执行，禁止按进程名结束 `Quattro.exe`，禁止读取、覆盖或删除正式版配置，禁止向正式版窗口发送测试消息。正式版与测试版并存验收必须证明正式版 PID、窗口和配置在测试前后保持不受影响。
- 自动化测试不得打开浏览器、资源管理器、终端、编辑器、文件选择器、打印对话框、邮件客户端或其它外部 GUI 程序。外部程序启动必须使用替身、记录器或只返回模拟结果的测试接口。
- 自动化测试产生的子进程必须逐个记录 PID、启动参数和归属标记；清理只能终止本次测试明确创建且身份验证匹配的进程。禁止使用按进程名批量结束、端口模糊匹配或窗口标题模糊匹配进行清理。
- 自动化测试不得锁定工作站、切换用户、切换虚拟桌面、最小化或排列用户窗口，不得调用会影响当前桌面布局的系统命令。测试窗口销毁后不得改变任何非测试窗口的位置、大小、可见性和 Z 序。
- 所有 GUI 自动化测试必须设置总超时、单步骤超时和无响应清理路径。超时后只能清理本次测试资源，不得通过重启 Explorer、结束用户应用、注销、重启系统或终止正式版进程解除阻塞。
- 自动化测试的日志、截图、转储和临时文件必须写入本次独立测试目录；禁止写入正式版目录、共享输出目录或其它测试的运行目录。
- 自动化测试执行打包时，打包输出目录必须位于本次独立测试目录中，且不得与其它自动化测试使用相同的打包输出目录。打包缓存可以在工具明确支持并发安全、缓存键能够隔离构建条件且不会混入测试输出或正式版产物的前提下条件性共享；不满足这些条件时必须使用本次测试独立缓存目录。
...
- 测试或验收检查更新安装链路、`QuattroUpdater`、`AppLaunchLocker`、Quattro 工具箱启动 `AppLaunchLocker`、独立进程启动、内嵌释放或相关 launcher/打包行为时，必须使用 `build.ps1 --all`（PowerShell 参数形式为 `-All`）构建完整组件包后再测试；禁止使用默认精简构建测试这些功能，否则缺少附属 EXE 或内嵌组件的结果只能视为构建范围不满足，不能作为产品缺陷或验收结论。
