...
- 范围刷新：启动项、普通标签和分组右键菜单均可刷新图标与跟踪菜单；标签和分组刷新仅覆盖其范围内的自定义启动项；菜单栏“刷新”相当于对所有分组执行一次分组刷新。
...
- 主题系统：通过主题文件统一控制文字、Panel、按钮、输入框、列表、Table、Link、Tooltip、TabControl、ToolBar、GroupBox、菜单、进度条和表单布局等公共组件样式；Panel 统一处理容器状态和滚动，Table 支持稳定行键、单行增量更新、当前处理行高亮框和动态行 Tooltip，ToolBar 支持动态项目、公共图标项与自动溢出，TabControl 统一处理导航条、页面内容布局及全层级页内快捷键导航，提供 `Standard`、`EmphasizedSegmented`、`MinimalUnderline`、`SoftPill`、`ConnectedTabs` 公共外观语义，并且所有外观均支持 `Horizontal` 和 `Vertical` 排列方向；标签组外层可通过 `ThemedTabControlOptions::containerStyle` 选择 `AppearanceDefault`、`Framed` 或 `Borderless`，在不增加页面私有样式的情况下控制整体容器外框。ComboBox 的项目和选中状态由公共语义接口维护，普通公共控件可通过 `ThemedUi::SetTooltip` 绑定统一悬停提示；界面通过 `ThemedUi` 语义接口创建控件。窗口尺寸、布局 metric、组件模板、主题消息框和运行时 `WM_DPICHANGED` 由 `ThemedWindowUi`/`ThemedUi` 按 96-DPI 逻辑像素统一处理，业务窗口不直接计算 DPI。
...
- `.quattro/context-menu.ini`：各构建共用的右键菜单跟踪设置。
- `.quattro/cache/shell-context-menu.bin`：各构建共用的右键菜单结构与菜单图标缓存。
- `icons/cache/`：运行时生成的启动项图标缓存。
...
- `tools/tabler-icons.json` 是图标语义名、glyph 码点和正式版子集裁剪的唯一清单；运行时代码通过主题公共 Tabler 图标接口加载和绘制字体。正式版从该清单生成子集字体，本地开发和测试默认使用完整字体；本地可通过 `tools/build.ps1 -SubsetTablerFont` 按“引用预检、生成子集、cmap/字体名称复检、嵌入包体”的正式顺序验证字体裁剪。
